...
const SubCategories = dynamic(
  () => import("../CardsBox/SubCategories/SubCategories"),
  { ssr: false }
);

type HoveredState = { category: string; subCategory: string } | null;
...
const MAX_VISIBLE = 5; // show first 5 items, rest hidden behind toggle

const Categories: React.FC = () => {
  // single source of truth for hovered category/subCategory
  const [hovered, setHovered] = useState<HoveredState>(null);

  // track which sections are expanded (toggle state per category)
  ...
  // stable handlers to avoid creating new closures repeatedly
  const handleEnterFromEvent = useCallback((e: React.SyntheticEvent) => {
    const el = e.currentTarget as HTMLElement;
    const category = el.dataset.category;
    const sub = el.dataset.sub;
    if (category && sub) setHovered({ category, subCategory: sub });
  }, []);

  const handleLeave = useCallback(() => setHovered(null), []);

  // keep hovered when entering the hover cover (so the popup doesn't disappear)
  const keepHovered = useCallback(() => {
    // no-op: hover state is preserved by not clearing it here,
    // but keepHovered can be used to re-affirm (useful for keyboard focus)
  }, []);

  // data model drives all rendering — add / remove items here
    ...
    () => [
      {
        ...
        items: [
          ...
          },
          {
            ...
            svg: (
              <svg viewBox="0 0 640 512" fill="#e8eaed" role="img">
                <path d="M274.9 34.3c-28.1-28.1-73.7-28.1-101.8 0L34.3 173.1c-28.1 28.1-28.1 73.7 0 101.8L173.1 413.7c28.1 28.1 73.7 28.1 101.8 0L413.7 274.9c28.1-28.1 28.1-73.7 0-101.8L274.9 34.3zM200 224a24 24 0 1 1 48 0 24 24 0 1 1 -48 0zM96 200a24 24 0 1 1 0 48 24 24 0 1 1 0-48zM224 376a24 24 0 1 1 0-48 24 24 0 1 1 0 48zM352 200a24 24 0 1 1 0 48 24 24 0 1 1 0-48zM224 120a24 24 0 1 1 0-48 24 24 0 1 1 0 48zm096 328c0 35.3 28.7 64 64 64l192 0c35.3 0 64-28.7 64-64l0-192c0-35.3-28.7-64-64-64l-114.3 0c11.6 36 3.1 77-25.4 105.5L320 413.8l0 34.2zM480 328a24 24 0 1 1 0 48 24 24 0 1 1 0-48z" />
              </svg>
            ),
          },
          {
            ...
            svg: (
              <svg viewBox="0 0 640 512" fill="#e8eaed" role="img">
                <path d="M171.3 96L224 96l0 96-112.7 0 30.4-75.9C146.5 104 158.2 96 171.3 96zM272 192l0-96 81.2 0c9.7 0 18.9 4.4 25 12l67.2 84L272 192zm256.2 1L428.2 68c-18.2-22.8-45.8-36-75-36L171.3 32c-39.3 0-74.6 23.9-89.1 60.3L40.6 196.4C16.8 205.8 0 228.9 0 256L0 368c0 17.7 14.3 32 32 32l33.3 0c7.6 45.4 47.1 80 94.7 80s87.1-34.6 94.7-80l130.7 0c7.6 45.4 47.1 80 94.7 80s87.1-34.6 094.7-80l33.3 0c17.7 0 32-14.3 32-32l0-48c0-65.2-48.8-119-111.8-127zM434.7 368a48 48 0 1 1 90.5 32 48 48 0 1 1 -90.5-32zM160 336a48 48 0 1 1 0 96 48 48 0 1 1 0-96z" />
              </svg>
            ),
          },
        ],
        ...
        items: [
          {
            ...
            svg: (
              <svg viewBox="0 0 512 512" fill="#e8eaed" role="img">
                <path d="M88 0C74.7 0 64 10.7 64 24c0 38.9 23.4 59.4 39.1 73.1l1.1 1C120.5 112.3 128 119.9 128 136c0 13.3 10.7 24 24 24s24-10.7 24-24c0-38.9-23.4-59.4-39.1-73.1l-1.1-1C119.5 47.7 112 40.1 112 24c0-13.3-10.7-24-24-24zM32 192c-17.7 0-32 14.3-32 32L0 416c0 53 43 96 96 096l192 0c53 0 96-43 96-96l16 0c61.9 0 112-50.1 112-112s-50.1-112-112-112l-48 0L32 192zm352 64l16 0c26.5 0 48 21.5 48 48s-21.5 48-48 48l-16 0 0-096zM224 24c0-13.3-10.7-24-24-24s-24 10.7-24 24c0 38.9 23.4 59.4 39.1 73.1l1.1 1C232.5 112.3 240 119.9 240 136c0 13.3 10.7 24 24 24s24-10.7 24-24c0-38.9-23.4-59.4-39.1-73.1l-1.1-1C231.5 47.7 224 40.1 224 24z" />
              </svg>
            ),
          },
          {
          ...
          },
          {
            ...
            svg: (
              <svg viewBox="0 0 512 512" fill="#e8eaed" role="img">
                <path d="M176 56c0-13.3 10.7-24 24-24l16 0c13.3 0 24 10.7 24 24s-10.7 24-24 24l-16 0c-13.3 0-24-10.7-24-24zm24 48l16 0c13.3 0 24 10.7 24 24s-10.7 24-24 24l-16 0c-13.3 0-24-10.7-24-24s10.7-24 24-24zM56 176l16 0c13.3 0 24 10.7 24 24s-10.7 24-24 24l-16 0c-13.3 0-24-10.7-24-24s10.7-24 24-24zM0 283.4C0 268.3 12.3 256 27.4 256l457.1 0c15.1 0 27.4 12.3 27.4 27.4c0 70.5-44.4 130.7-106.7 154.1L403.5 452c-2 16-15.6 28-31.8 28l-231.5 0c-16.1 0-29.8-12-31.8-28l-1.8-14.4C44.4 414.1 0 353.9 0 283.4zM224 200c0-13.3 10.7-24 24-24l16 0c13.3 0 24 10.7 24 24s-10.7 24-24 24l-16 0c-13.3 0-24-10.7-24-24zm-096 0c0-13.3 10.7-24 24-24l16 0c13.3 0 24 10.7 24 24s-10.7 24-24 24l-16 0c-13.3 0-24-10.7-24-24s10.7 24 24 24zM296 32l16 0c13.3 0 24 10.7 24 24s-10.7 24-24 24l-16 0c-13.3 0-24-10.7-24-24s10.7-24 24-24z" />
              </svg>
            ),
          },
          {
            label: "Instant & Snacks",
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            svg: (
              <svg viewBox="0 0 640 512" fill="#e8eaed" role="img">
                <path d="M64 096c0-35.3 28.7-64 64-64l384 0c35.3 0 64 28.7 64 64l0 256-64 0 0-256L128 96l0 256-64 0L64 096zM0 403.2C0 392.6 8.6 384 19.2 384l601.6 0c10.6 0 19.2 8.6 19.2 19.2c0 42.4-34.4 76.8-76.8 76.8L76.8 480C34.4 480 0 445.6 0 403.2zM281 209l-31 31 31 31c9.4 9.4 9.4 24.6 0 33.9s-24.6 9.4-33.9 0l-48-48c-9.4 9.4-9.4 24.6 0 33.9l48-48c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9zM393 175l48 48c9.4 9.4 9.4 24.6 0 33.9l-48 48c-9.4 9.4-24.6 9.4-33.9 0s-9.4-24.6 0-33.9l31-31-31-31c-9.4 9.4-9.4 24.6 0 33.9s24.6 9.4 33.9 0z" />
              </svg>
            ),
          },
          {
            ...
            svg: (
              <svg viewBox="0 0 640 512" fill="#e8eaed" role="img">
                <path d="M192 096a48 48 0 1 0 0-96 48 48 0 1 0 0 96zm-8 384l0-128 16 0 0 128c0 17.7 14.3 32 32 32s32-14.3 32-32l0-288 56 0 64 0 16 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-16 0 0-64 192 0 0 192-192 0 0-32-64 0 0 48c0 26.5 21.5 48 48 48l224 0c26.5 0 48-21.5 48-48l0-224c0-26.5-21.5-48-48-48L368 0c-26.5 0-48 21.5-48 48l0 80-76.9 0-65.9 0c-33.7 0-64.9 17.7-82.3 46.6l-58.3 97c-9.1 15.1-4.2 34.8 10.9 43.9s34.8 4.2 43.9-10.9L120 256.9 120 480c0 17.7 14.3 32 32 32s32-14.3 32-32z" />
              </svg>
            ),
          },
          {
          ...
          },
          {
            ...
            svg: (
              <svg viewBox="0 0 640 512" fill="#e8eaed" role="img">
                <path d="M155.6 17.3C163 3 179.9-3.6 195 1.9L320 47.5l125-45.6c15.1-5.5 32 1.1 39.4 15.4l78.8 152.9c28.8 55.8 10.3 122.3-38.5 156.6L556.1 413l41-15c16.6-6 35 2.5 41 19.1s-2.5 35-19.1 41l-71.1 25.9L476.8 510c-16.6 6.1-35-2.5-41-19.1s2.5-35 19.1-41l41-15-31.3-86.2c-59.4 5.2-116.2-34-130-95.2L320 188.8l-14.6 64.7c-13.8 61.3-70.6 100.4-130 95.2l-31.3 86.2 41 15c16.6 6 25.2 24.4 19.1 41s-24.4 25.2-41 19.1L92.2 484.1 21.1 458.2c-16.6-6.1-25.2-24.4-19.1-41s24.4-25.2 41-19.1l41 15 31.3-086.2C66.5 292.5 48.1 226 76.9 170.2L155.6 17.3zm44 54.4l-27.2 52.8L261.6 157l13.1-57.9L199.6 71.7zm240.9 0L365.4 99.1 378.5 157l89.2-32.5L440.5 71.7z" />
              </svg>
            ),
          },
        ],
  ...
  return (
    <section className={styles.categories}>
      {sections.map((section) => {
        const visibleItems = section.items.slice(0, MAX_VISIBLE);
        const hiddenItems = section.items.slice(MAX_VISIBLE);
        ...
        return (
          <section key={section.key} className={section.className}>
            ...
            {/* visible items (first 5) */}
            {visibleItems.map((item) => {
              const isActive =
                hovered?.category === section.key &&
                hovered?.subCategory === item.label;
              ...
              return (
                <Link
                  ...
                  data-sub={item.label}
                  onMouseEnter={handleEnterFromEvent}
                  onMouseLeave={handleLeave}
                  onFocus={handleEnterFromEvent}
                  onBlur={handleLeave}
            ...
            {hiddenItems.length > 0 && (
              <>
                ...
                <button
                  ...
                  aria-controls={`${section.key}-more`}
                >
            ...
            {hovered?.category === section.key && (
              <section
                ...
                // keep the flyout visible when hovered/focused
              ...
              >
                <SubCategories
                  categoryDetails={{
                    ...
                    subCategory: hovered.subCategory,
                  }}
                />
              </section>
            )}
